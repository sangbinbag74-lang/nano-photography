...
import { verifyUserPhoneAction } from "@/app/actions/auth";
...
const handleVerificationSuccess = async (phoneNumber: string) => {
    ...
    try {
        const result = await verifyUserPhoneAction(pendingUser.uid, phoneNumber);

        if (result.success) {
            alert(t.auth.phone_verification.success);
            setShowVerification(false);
            // Force a reload or state update if needed, but alert is enough for now
